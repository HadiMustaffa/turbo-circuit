...
export function createRenderer(canvas, opts = {}) {
  ...
  // Cheap camera-only update, with no render. The __game hook needs this: a headless driver
  // steps the sim thousands of ticks without drawing a frame, and anything it then asks about
  // screen position would otherwise be measured against a camera that never moved — producing
  // "0 of 8 karts on screen" for a game where every kart is visibly racing.
  function syncCamera(state, alpha = 0, o = {}) {
    if (!state || !track) return;
    updateCamera(state, alpha, {
      playerIndex: o.playerIndex ?? state.playerIndex ?? 0,
      camera: o.camera || 'chase',
      dt: o.dt || SIM.STEP,
    });
  ...
  return {
    init, draw, onResize, setQuality, dispose, syncCamera,

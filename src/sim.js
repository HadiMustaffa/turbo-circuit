...
function stepCountdown(state, inputs) {
  ...
  if (cdt <= 0) {
    ...
    for (const r of state.racers) {
      ...
      else if (r.rocketStart === 'early') { r.speed *= PHYSICS.rocketStart.bogSpeedMul; r.pulseTicks = PHYSICS.rocketStart.bogTicks; }
...
export function hashState(state) {
  ...
  for (const r of state.racers) {
    parts.push(
      ...
      r.boostTicks, r.spinTicks, r.overdriveTicks, r.finished ? 1 : 0, r.finishTick,
